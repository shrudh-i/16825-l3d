...
class VolumeRenderer(torch.nn.Module):
    ...
    def _compute_weights(
        ...
        # compute transmittance for each i
        for i in range(N):
            weights.append(T)
            T = torch.mul(T, (torch.exp(-torch.mul(rays_density[:,i,0], deltas[:,i,0]+eps))))

        # TODO (1.5): Compute weight used for rendering from transmittance and alpha
        weights = torch.stack(weights, dim=1).unsqueeze(-1)
        weights = torch.mul(weights, (1-torch.exp(-torch.mul(rays_density, deltas+eps))))
...
class SphereTracingRenderer(torch.nn.Module):
    ...
    def sphere_tracing(
        ...
        for _ in range(self.max_iters):
            ...
            mask = t.view(N, 1) <= 1e-6 # thresholding to check if it has hit the surface
            if torch.sum(mask) == N:
                break
...
def sdf_to_density(signed_distance, alpha, beta):
    ...
    laplace_dist = torch.distributions.laplace.Laplace(0, beta)

    # cdf -> cumulative distribution function = value b/w [0, 1]
    return alpha * laplace_dist.cdf(-signed_distance)
